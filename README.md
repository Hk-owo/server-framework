...
- **同步快速路径 + 显式异步** —— `server.Get(...)` 注册的 handler **直接在事件循环线程执行**：没有任务入队、没有线程唤醒、没有跨线程协程恢复。实测 `/json` 4 线程 100 连接约 **7.5 万 req/s**、8 线程 500 连接约 **8.4 万 req/s**。耗时 handler 用 `GetAsync`/`PostAsync` 注册，`QueryAwaiter` 把任务投进全局线程池，后台线程完成后经 `eventfd` 唤醒事件循环恢复协程。等待唤醒用的是 io_uring 的 **read**（由 read 把 eventfd 计数取走并清零）——若改用 `poll_add` 只监听可读而不消费计数，eventfd 从第一次写入起就永久可读，事件循环会空转一整个核。`resumePending` 的 CAS 保证同一协程不会被重复入队。
...
|------|----------|-------------|------------|----------------|---------------|
| `GET /json` | 59.7k | 72.4k | 75.4k | 77.9k | **84.2k** |
| `GET /api/user/42` | 62.1k | 73.3k | 76.2k | 78.9k | **84.9k** |
| `POST /echo` | 58.7k | 72.8k | 78.3k | 74.0k | 60.1k |
| `GET /async/status` | 5.9k | 6.2k | 6.2k | 6.2k | 6.2k |

（单位 req/s；20 个组合的 `connect/read/write/timeout` 与 `Non-2xx` 全部为 0，60 轮零崩溃。）

延迟：同步端点 P50 亚毫秒~8ms、P99 < 11ms；服务器 CPU——同步端点约 1.2~1.3 核，**异步端点仅 0.37~0.44 核**（`/async/status` 的 handler 只 sleep，几乎不耗 CPU）。
