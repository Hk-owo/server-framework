...
> **注**：下面的「端点 × 并发矩阵」已用当前实现重测（单实例 `SF_LOOPS=1`，机器空闲，
> 4 端点 × 5 级别 × 3 轮）。「多事件循环扩展性」那张表仍是改动前的数据——它要按实例数
> 反复绑核压测，对机器负担大，本次未重测。两张表的口径也不同：矩阵是单实例不绑核，
> 扩展性是服务端绑前 N 核。因此**不要跨表比较单实例绝对值**。
...
扩展是次线性的：8 实例时服务端只用约 2.7 核、压测端约 2.3 核，两者都没打满 12 核——说明瓶颈已经转移到 **loopback 网络栈与压测工具**（要测服务端上限需要多机压测）。**本表未随本轮改动重测**（按实例数反复绑核压测对机器负担较大），数字仅作扩展趋势参考；表中的单实例基线 136k 与本页矩阵表的单实例数字口径不同（绑核 vs 不绑核），不可直接对照。
