 ...
 */
template<typename T>
class MPSCQueue : public MPSCBase<T>{
private:
...
template<typename T>
void MPSCQueue<T>::wait_uring() noexcept {
...
template<typename T>
bool MPSCQueue<T>::dequeue(T &task) {
    bool res = MPSCBase<T>::dequeue(task);
    if(!res)
        peek_times++;
    else peek_times = 1;
    return res;
...
template<typename T>
void MPSCQueue<T>::peek_uring() noexcept {
    cqe = nullptr;
...
template<typename T>
void MPSCQueue<T>::notify_stop_uring() noexcept {
...
template<typename T>
void MPSCQueue<T>::wait_for_data_uring() noexcept {
...
template<typename T>
void MPSCQueue<T>::on_data_ready_uring() noexcept {
